...
import {
    ...
    getServiceTypeList,
    saveServiceType,
} from "/@/api/backend/order/service/type";
...
const refreshList = (params?: { query?: Query, loading?: boolean }) => {
    ...
    getServiceTypeList(params.query || queryInfo.value).then(res => {
...
const onAdd = () => {
    ElMessageBox.prompt("", "新增服务类型", {
        ...
        .then(({value}) => {
            saveServiceType({name: value}).then(() => {
...
const edit = (id: number) => {
    ElMessageBox.prompt("请修改服务类型名称", "编辑", {
        ...
        .then(({value}) => {
            saveServiceType({id, name: value}).then(() => {
                refreshList({loading: false});
            });
        });
};
