...
import {agentSeoApiTostopCheck} from "/@/api/backend/agent/seo/api/tostop/tostop";

const store = agentSeoApiStopConfig()
const pass = () => {
    agentSeoApiTostopCheck({
        id: store.id,
...
const reject = () => {
    ...
    ElMessageBox.prompt('请输入驳回理由', '驳回', {
        ...
        .then(({value}) => {
            agentSeoApiTostopCheck({id, status: 'reject', remark: value}).then(() => {
                store.refreshPage = true
                store.dialogVisible = false
            })
        })
