            ...
            <el-table v-loading="loading" :data="certList" border>
                ...
                <el-table-column
                    ...
                    <template #default="scope">
                        ...
                        <span v-else-if="scope.row.current_ranking <= 10"
                ...
                <el-table-column
                    ...
                    <template #default="scope">
                        ...
                        <span v-if="scope.row.change_ranking > 0"
...
import {agentSeoApiTostopDetail, agentSeoApiTostopList} from "/@/api/backend/agent/seo/api/tostop/tostop";
import BtnRefresh from "/@/components/buttons/BtnRefresh.vue";
import BtnSearch from "/@/components/buttons/BtnSearch.vue";
import {agentSeoApiStopConfig} from "/@/stores/agent/seo/api/tostop";
import PopupForm from "/@/views/backend/agent/seo/api/tostop/popupForm.vue";
import BtnEdit from "/@/components/buttons/BtnEdit.vue";
import {auth} from "/@/utils/common";
import BtnAudit from "/@/components/buttons/BtnAudit.vue";

const store = agentSeoApiStopConfig()
...
const operation = (id: number) => {
    agentSeoApiTostopDetail({id: id}).then(res => {
        store.custom_name = res.data.content.custom_name
        store.custom_contact_name = res.data.content.custom_contact_name
...
const getList = () => {
    pageInfo.status = activeName.value
    loading.value = true
    agentSeoApiTostopList(pageInfo).then(res => {
