...
import {uploadTaskRevision} from "/@/api/backend/open/panorama/order";
...
const beforePhotoUpload = (file: UploadRawFile) => {
    ...
    if (["image/*", "image/jpeg", "image/png", "image/gif"].indexOf(file.type) < 0) {
        ElMessage.error("上传图片只能是 JPG或者PNG 格式!");
        return false;
    }
...
const onSubmit = async (formEl: FormInstance | undefined) => {
    ...
    await formEl.validate((valid) => {
        if (valid) {
            ...
            uploadTaskRevision(uploadForm).then(() => {
                emit("onSuccess");
                closeDialog();
            }).finally(() => {
