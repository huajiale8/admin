...
import {
    disablePanoramaOrdersUrl,
    enablePanoramaOrdersUrl,
    getPanoramaOrdersUrlDetail,
    getPanoramaOrdersUrlList,
} from "/@/api/backend/open/panorama/orders";
import BtnSearch from "/@/components/buttons/BtnSearch.vue";
import PopupForm from "/@/views/backend/open/panorama/orders/popupForm.vue";
import {usePanoramaOrderStore} from "/@/stores/open/panorama/orders";
...
const store = usePanoramaOrderStore();
...
const refreshList = (params?: { query?: Query, loading?: boolean }) => {
    ...
    getPanoramaOrdersUrlList(params.query || queryInfo.value).then(res => {
...
const edit = (id: number) => {
    getPanoramaOrdersUrlDetail({id}).then(res => {
        store.status = "edit";
...
const disable = (id: number) => {
    return new Promise<void>((resolve, reject) => {
        disablePanoramaOrdersUrl({id}).then(() => {
            resolve();
        }).catch(() => {
            reject();
        });
...
const enable = (id: number) => {
    return new Promise<void>((resolve, reject) => {
        enablePanoramaOrdersUrl({id}).then(() => {
            resolve();
        }).catch(() => {
            reject();
        });
