...
import {getOrderQuery, updateTask} from "/@/api/backend/open/order";
...
const orderRules = reactive<FormRules>({
    pid: [
        {required: true, message: "请选择平面订单", trigger: "blur"},
    ],
    name: [
...
const getOrders = (query: string) => {
    if (query) {
        loading.value = true;
        getOrderQuery({name: query}).then(res => {
            loading.value = false;
        ...
        <el-scrollbar class="ba-table-form-scrollbar" ref="scrollbarRef">
            <div class="ba-operate-form">
                <el-form
                    ...
                    <el-form-item label="平面订单" prop="pid">
                        <el-select
