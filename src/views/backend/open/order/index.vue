...
import {
    ...
    getTaskOriginCheck,
    getTaskPackage,
    getTaskRevisionCheck,
    getTaskRevisionUpload,
    getTaskRevisionView,
    getTaskTaking,
    getTaskUpload,
    postTaskOnline,
    shootTaskOrigin,
} from "/@/api/backend/open/order";
import type {TabsPaneContext} from "element-plus";
import {ElMessageBox} from "element-plus";
import PopupForm from "/@/views/backend/open/order/popupForm.vue";
import DispatchDialog from "/@/views/backend/open/order/DispatchDialog.vue";
import {usePanoramaTaskStore} from "/@/stores/open/panorama/order";
import PackageDialog from "/@/views/backend/open/order/PackageDialog.vue";
import TakingDialog from "/@/views/backend/open/order/TakingDialog.vue";
import OriginUploadDialog from "/@/views/backend/open/order/OriginUploadDialog.vue";
import OriginCheckDialog from "/@/views/backend/open/order/OriginCheckDialog.vue";
import RevisionViewDialog from "/@/views/backend/open/order/RevisionViewDialog.vue";
import RevisionUploadDialog from "/@/views/backend/open/order/RevisionUploadDialog.vue";
import RevisionCheckDialog from "/@/views/backend/open/order/RevisionCheckDialog.vue";
import CustomPickDialog from "/@/views/backend/open/order/CustomPickDialog.vue";
...
const customPickDialogVisible = ref(false);
const task_list = ref([]);
...
const handleTabClick = (tab: TabsPaneContext) => {
    queryInfo.value.status = tab.props.name.toString();
...
const custom = (id: number) => {
    getTaskCustom({id}).then(res => {
        store.task = res.data.content.order;
        store.pictures = res.data.content.pictures;
        store.fileList = res.data.content.fileList;
        customPickDialogVisible.value = true;
    });
};
...
<template>
    <div class="default-main ba-table-box">
        ...
        <CustomPickDialog v-model="customPickDialogVisible" @on-success="refreshList()"/>
        <el-card>
            <div class="m-b-10">
                <el-row style="flex: 1">
                    <el-col :span="6">
            ...
            <el-row class="m-b-10">
                <el-col>
                    <el-tabs v-model="active" @tab-click="handleTabClick">
                        ...
                        <el-tab-pane label="待客户选片" :name="4"></el-tab-pane>
                        <el-tab-pane label="待修图" :name="6"></el-tab-pane>
                        <el-tab-pane label="成片审核" :name="7"></el-tab-pane>
                        <el-tab-pane label="成片重修" :name="8"></el-tab-pane>
                        <el-tab-pane label="待上线" :name="9"></el-tab-pane>
                        <el-tab-pane label="已上线" :name="10"></el-tab-pane>
                    </el-tabs>
                </el-col>
            </el-row>
            <el-table v-loading="tableLoading" :data="task_list" border>
                ...
                >
                    <template #default="scope">
                        ...
                        <span v-if="active === 4">
                            <el-button
                                v-if="auth('custom')" type="primary"
                                @click="custom(scope.row.id)">选底片</el-button>
                        </span>
                        <span v-if="active === 5">
                            <el-button
                                v-if="auth('upload')" type="primary"
