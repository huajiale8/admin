<template>
    <div class="nav-tabs" ref="tabScrollbarRef">
        <div
            ...
            @mousedown="MiddleKey($event,item)"
            class="ba-nav-tab"
...
const MiddleKey = (a: any, item: any) => {
    let key = a.button
    if (key === 0) {
        router.push(item)
    } else if (key === 1) {
        navTabs.closeTab(item)
        proxy.eventBus.emit('onTabViewClose', item)
        if (navTabs.state.activeRoute?.path === item.path) {
            toLastTab()
        } else {
