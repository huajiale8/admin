 ...
 * @param callback
 */
export function validatorUrl(rule: any, val: string, callback: Function) {
    const urlReg = /^\/{1}[a-z|\/]*$/
    if (!urlReg.test(val)) {
        return callback(new Error('URL地址格式不正确(如:/dashboard)'))
    }
    return callback();
}

export function validatorPhone(rule: any, val: string, callback: Function) {
    const urlReg = /^1(3|4|5|7|8)[0-9]{9}$/
    if (!urlReg.test(val)) {
